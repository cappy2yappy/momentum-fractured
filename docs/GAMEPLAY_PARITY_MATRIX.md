...
| Verticality | Elevated exits, wall movement, and an authored 1280×2160 Wind Relay Shaft with ascent/recovery lanes and ten deliberate anchors | Partial | Room 6 now proves multi-screen camera travel and explicit recovery structure, but the remaining generated rooms retain one-screen staircase layouts. | Does the three-screen climb read clearly at camera speed? Do recovery anchors compete with the intended route? Is the single Drone pressure fair? | P0 |
...
| Enemies | Rooms 5–11 use Echo/Drone mixes; Rooms 7 and 12 still use scaled Echo bosses | Partial | Room 5 Echoes now use authored territory bounds and forward ledge probes; Room 6 uses one positioned Drone. Generated Rooms 7–11 still place enemies formulaically, and there are no aquatic enemies or bespoke boss moves/phases. | Do bounded Echoes remain in-lane under pursuit and knockback? Does each encounter activate the room's geometry? | P0 |
| Room density | Six explicit scenes, including authored Surface Approach and Wind Relay Shaft; six generated route rooms remain | Partial | Rooms 5–6 now have distinct purposes and geometry. Rooms 7–12 still share generated boundaries, anchor patterns, environmental treatment, and progression logic. | Do the first two slice rooms feel cohesive but spatially distinct? Can each remaining room be replaced without breaking campaign reachability? | P0 |
...
`tests/alpha_08_smoke.gd` currently verifies scene loading, method presence, selected constants, run-animation slicing, anchor presence, rewards, ability save/load, dash momentum preservation, restartable melee windows, single health-event propagation, tether aim acceptance, obstruction rejection, collision-safe rope correction, and authored Room 5/6 structure. A separate Echo regression covers territory and ledge behavior. The smoke suite directly calls room death handlers to simulate progression. It does **not** verify:
